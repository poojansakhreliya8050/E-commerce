...
import ProductReviewCard from '../component/ProductReviewCard'
...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchReviewes = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_URL}/api/v1/review/getReviews/${productId}`);
        console.log(response.data);
        setReviewes(response.data);
      } catch (err) {
        console.log(err);
      }
    }
        ...
        product != null ?
          <div>
            ...
            <div className='mt-20 mx-5'>
              <h1 className='text-2xl font-bold text-center mx-4'>Product Reviews</h1>
              {
                reviewes != null ?
                  reviewes.map(review => <ProductReviewCard  review={review}/>) : <></>
              }
            </div>
